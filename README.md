...
Базовый город задаётся одной server-only переменной:

```dotenv
DEFAULT_CITY_SLUG=irkutsk
```

Название, падежи, регион и координаты автоматически берутся из опубликованной записи `cities` в Directus по `slug`. При смене переменной перезапустите сервер (для статически собранных страниц — выполните новую сборку). Выбранный объект базового города также синхронизируется в браузерный `localStorage` под ключом `default_city`.
...
Город из `DEFAULT_CITY_SLUG` доступен в корне без городского префикса, а его зеркальные URL перенаправляются на canonical-адреса. Остальные города доступны под собственными префиксами. Городской чат-кластер сохраняет единую схему `/{city}/chat-voditeley/**` для всех городов.
